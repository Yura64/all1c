...
#Область КлючевыеСловаПлатформы
	...
	Соответствие.Вставить("НОВЫЙЗАПРОС", "NEWQUERY");
...
//
Функция ПолучитьОбъектыКонфигурации(ТипОбъекта = Неопределено, БезИсключенных = Ложь, РасширениеКонфигурации = Неопределено) Экспорт
	...
	КлючКонтекста = СтрШаблон("ТаблицаОбъектыКонфигурации_%1_%2_%3", ТипОбъектаСтрокой, БезИсключенных,
...
Функция ПолучитьТекстМодуляДляПроверкиОрфографии(Знач ТекстМодуля)
	...
	// Отдельно заменяем все символы возврата каретки на пробелы.
	// Если это делать с помощью регулярных выражений, то по неизвестной причине процесс замены иногда зависает.
	ТекстМодуля = СтрЗаменить(ТекстМодуля, Символы.ВК, " ");
...
Функция ПолучитьТаблицуОбластейМодуля(Знач ТекстМодуля)
	...
	Если НЕ СтрНайтиПоМассиву(ТекстМодуля, МассивТекстКонецОбласти).Успешно Тогда
...
Функция ПолучитьТаблицуОбластейМодуля83(Знач ТекстМодуля)
	...
	МассивТекстОбластьКонецОбласти = СформироватьМассивРусскихИАнглийскихЗначений("Область|КонецОбласти");
	МассивШаблонПоиска = Новый Массив;
	Для Каждого ТекстОбластьКонецОбласти Из МассивТекстОбластьКонецОбласти Цикл
		МассивШаблонПоиска.Добавить(СтрШаблон("^[ \t]*#[ \t]*(?:%1).*", ТекстОбластьКонецОбласти));
	КонецЦикла;
	
	МассивТекстОбласть = СформироватьМассивРусскихИАнглийскихЗначений(ВРег("#Область"));
	МассивТекстКонецОбласти = СформироватьМассивРусскихИАнглийскихЗначений(ВРег("#КонецОбласти"));
	
	Для Счет = 0 По МассивШаблонПоиска.Количество() - 1 Цикл
		
		ШаблонПоиска = МассивШаблонПоиска[Счет];
		...
		ТекстОбласть = МассивТекстОбласть[Счет];
		ТекстКонецОбласти = МассивТекстКонецОбласти[Счет];
		...
		Для Каждого ЭлементМассива Из МассивРезультатов Цикл
			...
			Если СтрНачинаетсяС(ТекстОбластиВРег, ТекстОбласть) Тогда
			...
			ИначеЕсли СтрНачинаетсяС(ТекстОбластиВРег, ТекстКонецОбласти) Тогда
...
Функция ПолучитьТаблицуИнструкцийПрепроцессора(Знач ТекстМодуля)
	...
	МассивТекстДляЕслиКонецЕсли = СформироватьМассивРусскихИАнглийскихЗначений("Если|КонецЕсли");
	МассивШаблонПоиска = Новый Массив;
	Для Каждого ТекстДляЕслиКонецЕсли Из МассивТекстДляЕслиКонецЕсли Цикл
		МассивШаблонПоиска.Добавить(СтрШаблон("^[ \t]*#[ \t]*(?:%1).*", ТекстДляЕслиКонецЕсли));
	КонецЦикла;
	...
	Для Счет = 0 По МассивШаблонПоиска.Количество() - 1 Цикл
		...
		ТекстЕсли = МассивТекстЕсли[Счет];
		ТекстКонецЕсли = МассивТекстКонецЕсли[Счет];
		...
		Для Каждого ЭлементМассива Из МассивРезультатов Цикл
			...
			Если СтрНачинаетсяС(ТекстИнструкцииВРег, ТекстЕсли) Тогда
			...
			ИначеЕсли СтрНачинаетсяС(ТекстИнструкцииВРег, ТекстКонецЕсли) Тогда
				...
				Пока Истина Цикл
					...
					Для Каждого НомерПараметра Из МассивНомеровПараметров Цикл
						...
						Если НомерПараметра + 1 <= МассивПараметров.ВГраница() Тогда
							...
							Если МассивТекстЭтотОбъект.Найти(ИмяМодуля) <> Неопределено Тогда
...
Функция ПолучитьТаблицуУсловийЕслиКонецЕсли(ТекстМодуля)
	...
	МассивТекстЕслиИначеКонецЕсли = СформироватьМассивРусскихИАнглийскихЗначений("Если|Иначе|КонецЕсли");
	МассивШаблонПоиска = Новый Массив;
	Для Каждого ТекстЕслиИначеКонецЕсли Из МассивТекстЕслиИначеКонецЕсли Цикл
		МассивШаблонПоиска.Добавить(СтрШаблон("^[ \t]*(?:%1).*", ТекстЕслиИначеКонецЕсли));
	КонецЦикла;
	МассивТекстЕсли = СформироватьМассивРусскихИАнглийскихЗначений(ВРег("Если"));
	МассивТекстТогда = СформироватьМассивРусскихИАнглийскихЗначений(ВРег("Тогда"));
	...
	Для Счет = 0 По МассивШаблонПоиска.Количество() - 1 Цикл
		...
		ТекстЕсли = МассивТекстЕсли[Счет];
		ТекстТогда = МассивТекстТогда[Счет];
		ТекстИначеЕсли = МассивТекстИначеЕсли[Счет];
		ТекстИначе = МассивТекстИначе[Счет];
		ТекстКонецЕсли = МассивТекстКонецЕсли[Счет];
...
Функция ПолучитьТаблицуЦиклов(ТекстМодуля)
	...
	МассивТекстДляПокаКонецЦикла = СформироватьМассивРусскихИАнглийскихЗначений("Для|Пока|КонецЦикла");
	МассивШаблонПоиска = Новый Массив;
	Для Каждого ТекстДляПокаКонецЦикла Из МассивТекстДляПокаКонецЦикла Цикл
		МассивШаблонПоиска.Добавить(СтрШаблон("^[ \t]*(?:%1).*", ТекстДляПокаКонецЦикла));
	КонецЦикла;
	
	МассивТекстДля = СформироватьМассивРусскихИАнглийскихЗначений(ВРег("Для"));
	МассивТекстПока = СформироватьМассивРусскихИАнглийскихЗначений(ВРег("Пока"));
	...
	Для Счет = 0 По МассивШаблонПоиска.Количество() - 1 Цикл
		...
		ТекстДля = МассивТекстДля[Счет];
		ТекстПока = МассивТекстПока[Счет];
		ТекстЦикл = МассивТекстЦикл[Счет];
		ТекстКонецЦикла = МассивТекстКонецЦикла[Счет];
...
// Инициализирует таблицу обработчиков событий, которые могут находится в области "ОбработчикиСобытий".
//
// Возвращаемое значение:
//   ТаблицаЗначений - таблица обработчиков с колонками:
//     ТипОбъекта                  - Перечисление.ТипыОбъектов - тип объекта, для которого определяется набор обработчиков.
//     ТипМодуля                   - Перечисление.ИдентификаторыСодержанияОбъектов - тип модуля, для которого определяется набор обработчиков.
//     Область                     - Строка - имя области, для которой определяется набор обработчиков.
//     ОбластьАнгл                 - Строка - имя области на английсском, для которой определяется набор обработчиков.
//     Обработчик                  - Строка - имя обработчика.
//
